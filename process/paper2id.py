...
if __name__ == '__main__':
    ...
    chn_papers = list(pd.read_excel(ori_data_path + '大模型-中文文献-对齐.xlsx')['Title'])
    eng_papers = list(pd.read_excel(ori_data_path + '大模型-英文文献-对齐.xlsx')['Title'])
    ...
    chn_paper_vec = np.load(processed_data_path + '大模型-中文文献-对齐-加引用次数output_vectors.npz',)['arr_0']
    eng_paper_vec = np.load(processed_data_path + '大模型-英文文献-对齐-加引用次数output_vectors.npz')['arr_0']
