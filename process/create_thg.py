...
def get_patent_patent_data(data_path: str, patent2id: dict):
    ...
    data = pd.read_excel(data_path + 'data_v2.xlsx', usecols=['公开（公告）号', '引证专利', 'IPC']).rename(columns={'公开（公告）号': 'patent', '引证专利': 'inf_patent'})
    data = data.dropna(subset=['IPC'])
...
def get_patent_patent_edata(data_path: str, patent2id: dict) -> torch.Tensor:
    ...
    data2 = pd.read_excel(data_path + '10001-13517.xlsx', usecols=['公开（公告）号', '申请日', '引证专利', 'IPC']).rename(
        columns={'公开（公告）号': 'patent', '引证专利': 'inf_patent'})
    data = pd.concat([data1, data2]).dropna(subset=['IPC'])"""
    data = pd.read_excel(data_path + 'data_v2.xlsx', usecols=['公开（公告）号', '申请日', '引证专利', 'IPC']).rename(
        columns={'公开（公告）号': 'patent', '引证专利': 'inf_patent'})
    data = data.dropna(subset=['IPC'])
...
def prepare_ndata(processed_data_path: str) -> dict:
    ...
    ipc_embed_matrix = torch.load(processed_data_path + 'ipc_embed_matrix.pt')
...
if __name__ == '__main__':
    ...
    if thg.num_nodes('patent') < ndata['patent'].shape[0]:
        ndata['patent'] = ndata['patent'][:thg.num_nodes('patent')]
        #thg.add_nodes(ndata['patent'].shape[0]-thg.num_nodes('patent'), ntype='patent')
    ...
    print(new_thg)'''
    dgl.save_graphs(result_path + 'thg.dgl', [thg])
