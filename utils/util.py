...
def get_train_graph(graph_path=args.graph_path, timestamp=args.test_timestamp):
    ...
    result = dgl.to_bidirected(result)
    return result
...
def remove_edges_from_dgl2(dgl1, dgl2):
    # 获取两个图的边 (src 和 dst 代表源节点和目标节点)
    src1, dst1 = dgl1.edges()
    src2, dst2 = dgl2.edges()

    # 将边组成成对 (可以使用 zip 函数)
    edges1 = set(zip(src1.tolist(), dst1.tolist()))
    edges2 = set(zip(src2.tolist(), dst2.tolist()))

    # 找到公共的边
    common_edges = edges1 & edges2  # 取交集

    # 将公共边从 dgl1 和 dgl2 中删除
    if common_edges:
        # 分别获取源节点和目标节点
        src_to_remove, dst_to_remove = zip(*common_edges)

        # 从 dgl1 和 dgl2 中删除这些边
        dgl2.remove_edges(dgl2.edge_ids(src_to_remove, dst_to_remove))
...
def get_node_pairs(graphs, nodes: torch.Tensor):
    ...
    for tidx, t in enumerate(args.time_range):
        result_t = {}
        graph_t = graphs[tidx]
        graph_t = dgl.remove_self_loop(graph_t, etype='co-occurs with')
        graph_t = dgl.to_bidirected(graph_t)

        graphs_before_t = graphs[:tidx]
        for graph_before_t in graphs_before_t:
            graph_before_t = dgl.to_bidirected(graph_before_t)
            remove_edges_from_dgl2(graph_before_t,graph_t)
    ...
    for _ in range(args.min_timestamp, timestamp+1):
        ...
        for mtype in range(args.num_metapath_types):
            ...
            for node in nodes:
                ...
                metapaths_t = metapath_dict[node][_]
...
def create_data_splits(adj, next_adj, val_mask_fraction, test_mask_fraction):
    ...
    next_adj = next_adj - sp.dia_matrix((next_adj.diagonal()[np.newaxis, :], [0]),
                                        shape=next_adj.shape)  # Remove diagonal elements
    next_adj.eliminate_zeros()
    assert np.diag(next_adj.todense()).sum() == 0
    ...
    train_edges_all = sparse_to_tuple(adj)[0]
    train_edges_all = [tuple(row) for row in train_edges_all]
    ...
    """for e in edges_next:
        if e[0] < adj.shape[0] and e[1] < adj.shape[0]:
            edges.append(e)"""
    for e in edges_next:
        if tuple(e) not in train_edges_all:
            edges.append(e)
