    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in dataloader:
            ...
            for tidx, t in enumerate(args.time_range):
                ...
                embd_pu = embd_pu[:, tidx, :].view(-1, 1, embd_pu.shape[-1])
                embd_pv = embd_pv[:, tidx, :].view(-1, embd_pv.shape[-1], 1)
                embd_nu = embd_nu[:, tidx, :].view(-1, 1, embd_nu.shape[-1])
                embd_nv = embd_nv[:, tidx, :].view(-1, embd_nv.shape[-1], 1)
